...
data = {
    ...
    "Age": [25, 30, 28, 22, 24, 17, 29, 23, 16, 31],
...
df["Vehicles"] = ["Bike", "Car", "Bus", "Bike", "Car", "Bus", "Bike", "Car", "Bus", "Bike"] #adding a new column
df = df.assign(New_marks = df["Marks"] + 5) #adding a new column using assign method

df["New_marks"] = df["New_marks"] - 2 #updating an existing column

df["Categories"] = np.where(df["Age"] >= 18, "Adult", "Teenager") #adding a column based on condition

df = df.drop("City", axis=1) #deleting a column (single)

df = df.drop(["Grade", "Passed"], axis=1) #deleting multiple columns
